...
class Proj4phpProjTmerc  extends Proj4phpProj  {
  function init() {
    ...
    $this->ml0 = $this->a * $this->proj4php->common->mlfn($this->e0, $this->e1, $this->e2, $this->e3, $this->lat0);
  }
  ...
  */
  function forward($p) {
    ...
    $delta_lon = $this->proj4php->common->adjust_lon($lon - $this->long0); // Delta longitude
    $con;    // cone constant
    $x;$y;
    $sin_phi=sin($lat);
    $cos_phi=cos($lat);
    ...
    if ($this->sphere) {  /* spherical form */
      ...
      $n   = $this->a / sqrt($con);
      $ml  = $this->a * $this->proj4php->common->mlfn($this->e0, $this->e1, $this->e2, $this->e3, lat);
  ...
  */
  function inverse($p) {
    ...
    if ($this->sphere) {   /* spherical form */
      ...
      $lat = $this->proj4php->common->asinz($con);
      if ($temp < 0)
        $lat = -$lat;
      if (($g == 0) && ($h == 0)) {
        $lon = $this->long0;
      } else {
        $lon = $this->proj4php->common->adjust_lon(atan2($g,$h) + $this->long0);
      }
      ...
      for ($i=0;true;$i++) {
        ...
        $phi += $delta_phi;
        if (abs($delta_phi) <= $this->proj4php->common->EPSLN) break;
        if ($i >= $max_iter) {
          Proj4php::reportError("tmerc:inverse: Latitude failed to converge");
          return(95);
        }
      } // for()
      if (abs($phi) < $this->proj4php->common->HALF_PI) {
        ...
        $lon = $this->proj4php->common->adjust_lon($this->long0 + ($d * (1.0 - $ds / 6.0 * (1.0 + 2.0 * $t + $c - $ds / 20.0 * (5.0 - 2.0 * $c + 28.0 * $t - 3.0 * $cs + 8.0 * $this->ep2 + 24.0 * $ts))) / $cos_phi));
      } else {
        $lat = $this->proj4php->common->HALF_PI * $this->proj4php->common.sign($y);
        $lon = $this->long0;
      }
    }
    $p->x = $lon;
...
$this->proj['tmerc'] = new Proj4phpProjTmerc('',$this);